            ...
            >
                ...
                <div className='w-full h-auto flex flex-col px-5 md:px-10 xl:px-20 mt-10 gap-4 md:gap-8 lg:gap-10'>
                    <div className='w-full h-auto flex justify-center items-center relative rounded-2xl overflow-hidden sm:h-[300px] lg:h-[400px]'>
                        <img src={images[0]} alt="single donation image" className='rounded-2xl w-full h-full object-cover' />
                        ...
                        <div className='w-full h-auto flex flex-col absolute gap-2 lg:gap-4'>
                            <h1 className='text-white text-center font-prata text-sm sm:text-xl lg:text-2xl xl:text-4xl'>{singleDonation?.title}</h1>
                            <p className='text-white font-nunito text-center text-xs sm:px-10 sm:text-sm md:px-20 md:text-base lg:px-40 xl:px-60'>{singleDonation?.description?.slice(0, 150)}</p>
                            <div className='w-full h-auto flex justify-center items-center'>
                                <button className='px-4 py-2 bg-[#EB852C] text-white font-poppins rounded-3xl text-sm lg:py-3 lg:text-base lg:px-6 md:hover:bg-[#fd8721]'>Donate Now</button>
                            </div>
                        </div>
                    </div>
                    <div className='w-full h-auto flex flex-col mb-8 md:px-10'>
                        <p className='text-[#EB852C] text-sm font-prata text-center sm:text-base md:text-lg lg:text-xl'>{singleDonation?.description}</p>
                    </div>
                </div>
            </div>
                ...
                <div className='w-full h-auto flex flex-col'>
                    {
                        singleDonation?.donationsCategory?.map((donation, index) => (
                            <div key={index} className='w-full h-auto flex flex-col my-4 gap-2'>
                                <h1 className='my-4 lg:my-6 w-full h-auto text-lg text-center text-[#444444] sm:text-xl font-nunito xl:text-2xl font-semibold'>{donation.title}</h1>
