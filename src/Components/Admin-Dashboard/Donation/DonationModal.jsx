...
const CreateDonationForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const { title, description, image, startDate, endDate, donationsCategory } =
      formData;
    ...
    donationsCategory.forEach((category, categoryIndex) => {
      formPayload.append(
        `donationsCategory[${categoryIndex}][title]`,
        category.title
      );
    ...
    image.forEach((img) => {
      formPayload.append("image", img);
    });

    console.log("FormPayload Content:");
  for (let [key, value] of formPayload.entries()) {
    console.log(key, value);
  }

    try {
      await axios.post(`${backend}/admin/donation/create`, formPayload, {
