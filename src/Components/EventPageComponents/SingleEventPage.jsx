...
function SingleEventPage() {
    ...
    async function fetchSingleEvent() {
        try {
            ...
            const relatedEvents = response.data.data.filter(event => event._id !== id)
    ...
    return (
        ...
        >
            ...
            <div className='w-full h-auto flex flex-col my-10 px-5 md:px-10 xl:px-20'>
                <h1 className='font-prata text-2xl sm:text-3xl md:text-4xl lg:text-5xl'>{singleEvent?.title}</h1>
                ...
                <section className="w-full mt-12">
                    <div className="w-full flex flex-col md:flex-row gap-10">
                        <div className=" w-full md:w-2/3 ">
                            <p className='font-poppins md:text-lg' style={{ whiteSpace: 'pre-line' }} dangerouslySetInnerHTML={{ __html: singleEvent?.description }}></p>
                        </div>
                        {/* Related Events */}
